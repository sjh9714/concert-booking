...
![CI](https://github.com/jinhyuk9714/concert-booking/actions/workflows/ci.yml/badge.svg)
...
| Observability metrics | 예매 실패 사유, queue token abuse, outbox backlog, stock mismatch 관측 |
...
D/E/F k6 시나리오는 script만 추가된 상태이며, 정식 부하 결과는 pending입니다.
...
Redis stock은 빠른 선검증용 캐시입니다. 최종 진실은 DB입니다.
...
> 이 섹션은 기본적인 관측 지표를 설명합니다. alerting, dashboard, tracing, SLO 운영 체계까지 구현했다는 의미는 아닙니다. 실제 Prometheus scrape 구성에는 bearer token 또는 internal network/auth 정책이 필요합니다.
...
| D/E/F 시나리오 | script added, result pending 상태입니다. |
...
| Observability | 기본 metric 노출까지 구현했습니다. alerting, dashboard, tracing, SLO 운영 체계는 별도 과제입니다. |
...
| [docs/PERF_RESULT.md](docs/PERF_RESULT.md) | k6 측정 결과와 pending 시나리오 |
