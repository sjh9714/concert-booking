...
**"1만 명이 동시에 1,000석을 예매한다면, 데이터 정합성을 어떻게 검증할 것인가?"**
...
> 파일: `src/main/resources/db/migration/V*.sql`
...
spring:
  jpa:
    hibernate:
      ddl-auto: none        # 테이블 자동 생성 안 함 (Flyway로 관리)
    properties:
      hibernate:
  ...
  sql:
    init:
      mode: never            # schema.sql 자동 실행 비활성화
...
### Q3. 왜 Flyway migration으로 테이블을 만드나요?
`ddl-auto: update`는 컬럼 삭제/이름 변경을 감지 못하고, 인덱스를 자동 생성하지 않습니다.
Flyway versioned SQL을 사용하면 정확히 어떤 DDL이 어떤 순서로 실행됐는지 추적할 수 있습니다.
