...
| 항목 | 현재 상태 | 다음 보강 |
| --- | --- | --- |
| 세 시나리오 장기 반복 통계 | 세 전략 x 3회 formal local repeat는 보존했지만 평균/표준편차/신뢰구간은 주장하지 않음 | 동일 fixture에서 더 긴 반복 실행과 raw output, 환경, 해석을 함께 보존 |
| 세 시나리오 운영 성능 비교 | 세 시나리오는 branch/threshold 시나리오 검증으로만 보존 | 운영 latency/throughput/error-rate를 주장하려면 별도 조건과 반복 통계를 보존 |
| 대기열 token abuse 검증 (Scenario F) 부하 성능 | queue token abuse는 branch/threshold 검증 중심으로 보존 | 더 큰 VU 조건에서 unauthorized success 0과 latency를 함께 기록 |
...
> 정합성 포트폴리오입니다. 결제/만료 race 검증(Scenario D), 중복 요청 idempotency replay/conflict 검증(Scenario E),
> 대기열 token abuse 검증(Scenario F)은 세 전략 x 3회 local repeat로 branch/threshold를 검증했지만,
